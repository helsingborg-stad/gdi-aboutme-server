...
describe('email verification: GET /api/v1/aboutme/verify/phone/{code}', () => {
	// wrap full request and return response
	const requestVerifyPhone = async (db: object, verificationCode: string) => {
		let result: ReturnType<request.get>
		await createTestApp(db)
			.run(async server => {
				result = await request(server)
					.get(`/api/v1/aboutme/verify/phone/${verificationCode}`)
			})
		return result
	}

	it('ignores bad/missing codes and responds with {verified: false}', async () => {
		const { status, body } = await requestVerifyPhone({}, 'missing-verification-code')
	...
	it('updates existing person and responds with {verified: true}', async () => {
		...
		const { status, body } = await requestVerifyPhone(db, 'test-verification-code')
		expect(status).toBe(StatusCodes.OK)
		expect(body).toMatchObject({
			verified: true,
		})
